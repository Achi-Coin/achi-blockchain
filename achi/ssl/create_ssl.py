...
def get_achi_ca_crt_key() -> Tuple[Any, Any]:
    crt = pkg_resources.resource_string(__name__, "achi_ca.crt")
    key = pkg_resources.resource_string(__name__, "achi_ca.key")
    return crt, key
...
def generate_ca_signed_cert(ca_crt: bytes, ca_key: bytes, cert_out: Path, key_out: Path):
    ...
    new_subject = x509.Name(
        [
            x509.NameAttribute(NameOID.COMMON_NAME, "Achi"),
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Achi"),
            x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, "Honest Farming Division"),
        ]
    )
    ...
    cert = (
        ...
        .add_extension(
            x509.SubjectAlternativeName([x509.DNSName("achicoin.org")]),
            critical=False,
        )
...
def make_ca_cert(cert_path: Path, key_path: Path):
    ...
    subject = issuer = x509.Name(
        [
            x509.NameAttribute(NameOID.ORGANIZATION_NAME, "Achi"),
            x509.NameAttribute(NameOID.COMMON_NAME, "Achi CA"),
            x509.NameAttribute(NameOID.ORGANIZATIONAL_UNIT_NAME, "Honest Farming Division"),
        ]
    )
    root_cert = (
...
def main():
    return make_ca_cert(Path("./achi_ca.crt"), Path("./achi_ca.key"))
