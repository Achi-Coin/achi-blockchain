...
from achi.ssl.create_ssl import generate_ca_signed_cert, get_achi_ca_crt_key, make_ca_cert
from achi.util.bech32m import encode_puzzle_hash
from achi.util.config import (
    create_default_achi_config,
    initial_config_file,
    load_config,
    save_config,
...
def create_all_ssl(root: Path):
    ...
    achi_ca_crt, achi_ca_key = get_achi_ca_crt_key()
    achi_ca_crt_path = ca_dir / "achi_ca.crt"
    achi_ca_key_path = ca_dir / "achi_ca.key"
    achi_ca_crt_path.write_bytes(achi_ca_crt)
    achi_ca_key_path.write_bytes(achi_ca_key)
    ...
    achi_ca_crt, achi_ca_key = get_achi_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, achi_ca_crt, achi_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path):
    if create_certs is not None:
        if root_path.exists():
            ...
            print("** Please run `achi init` to migrate or create new config files **")
    else:
        return achi_init(root_path)


def achi_version_number() -> Tuple[str, str, str, str]:
...
def achi_minor_release_number():
    res = int(achi_version_number()[2])
    print(f"Install release number: {res}")
    return res


def achi_full_version_str() -> str:
    major, minor, patch, dev = achi_version_number()
    return f"{major}.{minor}.{patch}{dev}"


def achi_init(root_path: Path):
    if os.environ.get("ACHI_ROOT", None) is not None:
        print(
            f"warning, your ACHI_ROOT is set to {os.environ['ACHI_ROOT']}. "
            f"Please unset the environment variable and run achi init again\n"
            f"or manually migrate config.yaml"
        )

    print(f"Achi directory {root_path}")
    if root_path.is_dir() and Path(root_path / "config" / "config.yaml").exists():
        # This is reached if ACHI_ROOT is set, or if user has run achi init twice
        # before a new update.
    ...
    check_keys(root_path)
    print("")
    print("To see your keys, run 'achi keys show --show-mnemonic-seed'")
