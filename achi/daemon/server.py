...
from achi.cmds.init_funcs import achi_init
from achi.daemon.windows_signal import kill
from achi.server.server import ssl_context_for_root, ssl_context_for_server
from achi.ssl.create_ssl import get_mozzila_ca_crt
from achi.util.achi_logging import initialize_logging
from achi.util.config import load_config
from achi.util.json_util import dict_to_json_str
from achi.util.path import mkdir
from achi.util.service_groups import validate_service
from achi.util.setproctitle import setproctitle
from achi.util.ws_message import WsRpcMessage, create_payload, format_response
...
try:
    ...
    print("Error: Make sure to run . ./activate from the project folder before starting Achi.")
    quit()
...
service_plotter = "achi plots create"
...
if getattr(sys, "frozen", False):
    name_map = {
        "achi": "achi",
        "achi_wallet": "start_wallet",
        "achi_full_node": "start_full_node",
        "achi_harvester": "start_harvester",
        "achi_farmer": "start_farmer",
        "achi_introducer": "start_introducer",
        "achi_timelord": "start_timelord",
        "achi_timelord_launcher": "timelord_launcher",
        "achi_full_node_simulator": "start_simulator",
    }
...
class WebSocketServer:
    ...
    async def exit(self) -> Dict[str, Any]:
        ...
        log.info("achi daemon exiting in 5 seconds")
...
def launch_plotter(root_path: Path, service_name: str, service_array: List[str], id: str):
    # we need to pass on the possibly altered ACHI_ROOT
    os.environ["ACHI_ROOT"] = str(root_path)
...
def launch_service(root_path: Path, service_command) -> Tuple[subprocess.Popen, Path]:
    ...
    log.debug(f"Launching service with ACHI_ROOT: {os.environ['ACHI_ROOT']}")
