...
from achi.util.generator_tools import (
    additions_for_npc,
...
async def validate_block_body(
    ...
    if height > 0:
        ...
        timelord_coin = create_timelord_coin(
            ...
            uint64(calculate_timelord_reward(prev_transaction_block.height)),
    ...
    if (
        ...
        if block.transactions_generator is not None:
            ...
            # 7. Check that cost <= MAX_BLOCK_COST_ALVM
            log.debug(
                f"Cost: {cost} max: {constants.MAX_BLOCK_COST_ALVM} "
                f"percent full: {round(100 * (cost / constants.MAX_BLOCK_COST_ALVM), 2)}%"
            )
            if cost > constants.MAX_BLOCK_COST_ALVM:
                return Err.BLOCK_COST_EXCEEDS_MAX, None

            # 8. The ALVM program must not return any errors
            ...
            while curr.height > fork_h:
                ...
                if curr.transactions_generator is not None:
                    ...
                    curr_npc_result = get_name_puzzle_conditions(
                        curr_block_generator, min(constants.MAX_BLOCK_COST_ALVM, curr.transactions_info.cost), False
                    )
