...
from achi.protocols import wallet_protocol
from achi.protocols.full_node_protocol import RequestProofOfWeight, RespondProofOfWeight
from achi.protocols.protocol_message_types import ProtocolMessageTypes
from achi.protocols.wallet_protocol import (
...
from achi.util.ints import uint32, uint128
from achi.util.keychain import Keychain
from achi.util.lru_cache import LRUCache
from achi.util.merkle_set import MerkleSet, confirm_included_already_hashed, confirm_not_included_already_hashed
from achi.util.path import mkdir, path_from_root
from achi.wallet.block_record import HeaderBlockRecord
from achi.wallet.derivation_record import DerivationRecord
from achi.wallet.settings.settings_objects import BackupInitialized
from achi.wallet.transaction_record import TransactionRecord
from achi.wallet.util.backup_utils import open_backup_file
from achi.wallet.util.wallet_types import WalletType
from achi.wallet.wallet_action import WalletAction
from achi.wallet.wallet_blockchain import ReceiveBlockResult
from achi.wallet.wallet_state_manager import WalletStateManager
...
class WalletNode:
    ...
    def get_key_for_fingerprint(self, fingerprint: Optional[int]):
        ...
        if len(private_keys) == 0:
            self.log.warning("No keys present. Create keys with the UI, or with the 'achi keys' program.")
            return None
    ...
    async def on_connect(self, peer: WSAchiConnection):
        if self.wallet_state_manager is None or self.backup_initialized is False:
            return None
    ...
    async def complete_blocks(self, header_blocks: List[HeaderBlock], peer: WSAchiConnection):
    ...
    async def new_peak_wallet(self, peak: wallet_protocol.NewPeakWallet, peer: WSAchiConnection):
        if self.wallet_state_manager is None:
            return None
    ...
    async def _sync(self) -> None:
        ...
        peers: List[WSAchiConnection] = self.server.get_full_node_connections()
        if len(peers) == 0:
    ...
    async def get_additions(self, peer: WSAchiConnection, block_i, additions) -> Optional[List[Coin]]:
        if len(additions) > 0:
    ...
    async def get_removals(self, peer: WSAchiConnection, block_i, additions, removals) -> Optional[List[Coin]]:
        assert self.wallet_state_manager is not None
