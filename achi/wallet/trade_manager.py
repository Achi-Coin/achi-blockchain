...
from achi.types.coin_solution import CoinSolution
...
from achi.wallet.cc_wallet import cc_utils
from achi.wallet.cc_wallet.cc_utils import CC_MOD, SpendableCC, spend_bundle_for_spendable_ccs, uncurry_cc
from achi.wallet.cc_wallet.cc_wallet import CCWallet
from achi.wallet.puzzles.genesis_by_coin_id_with_0 import genesis_coin_id_for_genesis_coin_checker
from achi.wallet.trade_record import TradeRecord
from achi.wallet.trading.trade_status import TradeStatus
from achi.wallet.trading.trade_store import TradeStore
from achi.wallet.transaction_record import TransactionRecord
from achi.wallet.util.trade_utils import (
...
from achi.wallet.util.transaction_type import TransactionType
from achi.wallet.util.wallet_types import WalletType
from achi.wallet.wallet import Wallet
from achi.wallet.wallet_coin_record import WalletCoinRecord

# from achi.wallet.cc_wallet.debug_spend_bundle import debug_spend_bundle
            ...
            for id in offer.keys():
                ...
                if isinstance(wallet, CCWallet):
                    ...
                    if spend_bundle is None:
                        ...
                        to_exclude = spend_bundle.removals()
                    new_spend_bundle = await wallet.create_spend_bundle_relative_achi(amount, to_exclude)
                else:
    ...
    async def respond_to_offer(self, file_path: Path) -> Tuple[bool, Optional[TradeRecord], Optional[str]]:
        ...
        for coinsol in offer_spend_bundle.coin_solutions:
            ...
            if r:
                ...
                if coinsol.coin in [record.coin for record in unspent]:
                    return False, None, "can't respond to own offer"
                if achi_discrepancy is None:
                    achi_discrepancy = get_output_discrepancy_for_puzzle_and_solution(coinsol.coin, puzzle, solution)
                else:
                    achi_discrepancy += get_output_discrepancy_for_puzzle_and_solution(coinsol.coin, puzzle, solution)
                coinsols.append(coinsol)

        achi_spend_bundle: Optional[SpendBundle] = None
        if achi_discrepancy is not None:
            achi_spend_bundle = await self.wallet_state_manager.main_wallet.create_spend_bundle_relative_achi(
                achi_discrepancy, []
            )
            if achi_spend_bundle is not None:
                for coinsol in coinsols:
                    achi_spend_bundle.coin_solutions.append(coinsol)
        ...
        for colour in cc_discrepancies.keys():
            if cc_discrepancies[colour] < 0:
                ...
                if achi_spend_bundle is None:
                    to_exclude: List = []
                else:
                    to_exclude = achi_spend_bundle.removals()
                ...
                if my_cc_spends is None or my_cc_spends == set():
                    ...
                    if zero_spend_bundle is None:
                        return (
                            False,
                            None,
                            "Unable to generate zero value coin. Confirm that you have achi available",
                        )
        ...
        now = uint64(int(time.time()))
        if achi_spend_bundle is not None:
            spend_bundle = SpendBundle.aggregate([spend_bundle, achi_spend_bundle])
            # debug_spend_bundle(spend_bundle)
            if achi_discrepancy < 0:
                tx_record = TransactionRecord(
                    confirmed_at_height=uint32(0),
                    created_at_time=now,
                    to_puzzle_hash=token_bytes(),
                    amount=uint64(abs(achi_discrepancy)),
                    fee_amount=uint64(0),
                    confirmed=False,
                    sent=uint32(10),
                    spend_bundle=achi_spend_bundle,
                    additions=achi_spend_bundle.additions(),
                    removals=achi_spend_bundle.removals(),
                    wallet_id=uint32(1),
                    sent_to=[],
                    trade_id=std_hash(spend_bundle.name() + bytes(now)),
                    type=uint32(TransactionType.OUTGOING_TRADE.value),
                    name=achi_spend_bundle.name(),
                )
            else:
                tx_record = TransactionRecord(
                    confirmed_at_height=uint32(0),
