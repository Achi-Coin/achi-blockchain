...
from achi.types.announcement import Announcement
...
from achi.wallet.puzzles.puzzle_utils import (
...
from achi.wallet.secret_key_store import SecretKeyStore
from achi.wallet.sign_coin_solutions import sign_coin_solutions
...
class Wallet:
    ...
    async def get_max_send_amount(self, records=None):
        ...
        max_cost = self.wallet_state_manager.constants.MAX_BLOCK_COST_ALVM / 5  # avoid full block TXs
        current_cost = 0
        total_amount = 0
        total_coin_count = 0
    ...
    async def sign_transaction(self, coin_solutions: List[CoinSolution]) -> SpendBundle:
        return await sign_coin_solutions(
            ...
            self.wallet_state_manager.constants.MAX_BLOCK_COST_ALVM,
        )
    ...
    async def create_spend_bundle_relative_achi(self, achi_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None
        ...
        if achi_amount < 0:
            utxos = await self.select_coins(abs(achi_amount), exclude)
        else:
        ...
        achi_amount = spend_value + achi_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": achi_amount}]
