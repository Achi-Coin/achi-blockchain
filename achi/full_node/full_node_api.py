...
import achi.server.ws_connection as ws
from achi.consensus.block_creation import create_unfinished_block
from achi.consensus.block_record import BlockRecord
from achi.consensus.pot_iterations import calculate_ip_iters, calculate_iterations_quality, calculate_sp_iters
from achi.full_node.bundle_tools import best_solution_generator_from_template, simple_solution_generator
from achi.full_node.full_node import FullNode
from achi.full_node.mempool_check_conditions import get_puzzle_and_solution_for_coin
from achi.full_node.signage_point import SignagePoint
from achi.protocols import farmer_protocol, full_node_protocol, introducer_protocol, timelord_protocol, wallet_protocol
from achi.protocols.full_node_protocol import RejectBlock, RejectBlocks
from achi.protocols.protocol_message_types import ProtocolMessageTypes
from achi.protocols.wallet_protocol import PuzzleSolutionResponse, RejectHeaderBlocks, RejectHeaderRequest
from achi.server.outbound_message import Message, make_msg
from achi.types.blockchain_format.coin import Coin, hash_coin_list
from achi.types.blockchain_format.pool_target import PoolTarget
from achi.types.blockchain_format.program import Program
from achi.types.blockchain_format.sized_bytes import bytes32
from achi.types.coin_record import CoinRecord
from achi.types.end_of_slot_bundle import EndOfSubSlotBundle
from achi.types.full_block import FullBlock
from achi.types.generator_types import BlockGenerator
from achi.types.mempool_inclusion_status import MempoolInclusionStatus
from achi.types.mempool_item import MempoolItem
from achi.types.peer_info import PeerInfo
from achi.types.unfinished_block import UnfinishedBlock
from achi.util.api_decorators import api_request, peer_required, bytes_required, execute_task
from achi.util.generator_tools import get_block_header
from achi.util.hash import std_hash
from achi.util.ints import uint8, uint32, uint64, uint128
from achi.util.merkle_set import MerkleSet
...
class FullNodeAPI:
    ...
    @peer_required
    @api_request
    async def request_peers(self, _request: full_node_protocol.RequestPeers, peer: ws.WSAchiConnection):
    ...
    async def respond_peers(
        self, request: full_node_protocol.RespondPeers, peer: ws.WSAchiConnection
    ) -> Optional[Message]:
    ...
    async def respond_peers_introducer(
        self, request: introducer_protocol.RespondPeersIntroducer, peer: ws.WSAchiConnection
    ) -> Optional[Message]:
    ...
    @peer_required
    @api_request
    async def new_peak(self, request: full_node_protocol.NewPeak, peer: ws.WSAchiConnection) -> Optional[Message]:
        """
    ...
    async def new_transaction(
        self, transaction: full_node_protocol.NewTransaction, peer: ws.WSAchiConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_signage_point_or_end_of_sub_slot(
        self, new_sp: full_node_protocol.NewSignagePointOrEndOfSubSlot, peer: ws.WSAchiConnection
    ) -> Optional[Message]:
        # Ignore if syncing
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_signage_point(
        self, request: full_node_protocol.RespondSignagePoint, peer: ws.WSAchiConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def respond_end_of_sub_slot(
        self, request: full_node_protocol.RespondEndOfSubSlot, peer: ws.WSAchiConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def declare_proof_of_space(
        self, request: farmer_protocol.DeclareProofOfSpace, peer: ws.WSAchiConnection
    ) -> Optional[Message]:
        """
    ...
    async def signed_values(
        self, farmer_request: farmer_protocol.SignedValues, peer: ws.WSAchiConnection
    ) -> Optional[Message]:
        """
    ...
    async def new_infusion_point_vdf(
        self, request: timelord_protocol.NewInfusionPointVDF, peer: ws.WSAchiConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_signage_point_vdf(
        self, request: timelord_protocol.NewSignagePointVDF, peer: ws.WSAchiConnection
    ) -> None:
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    async def new_end_of_sub_slot_vdf(
        self, request: timelord_protocol.NewEndOfSubSlotVDF, peer: ws.WSAchiConnection
    ) -> Optional[Message]:
        if self.full_node.sync_store.get_sync_mode():
            return None
        if (
    ...
    async def request_puzzle_solution(self, request: wallet_protocol.RequestPuzzleSolution) -> Optional[Message]:
        ...
        error, puzzle, solution = get_puzzle_and_solution_for_coin(
            block_generator, coin_name, self.full_node.constants.MAX_BLOCK_COST_ALVM
        )
    ...
    @peer_required
    @api_request
    async def new_compact_vdf(self, request: full_node_protocol.NewCompactVDF, peer: ws.WSAchiConnection):
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    @peer_required
    @api_request
    async def request_compact_vdf(self, request: full_node_protocol.RequestCompactVDF, peer: ws.WSAchiConnection):
        if self.full_node.sync_store.get_sync_mode():
            return None
    ...
    @peer_required
    @api_request
    async def respond_compact_vdf(self, request: full_node_protocol.RespondCompactVDF, peer: ws.WSAchiConnection):
        if self.full_node.sync_store.get_sync_mode():
            return None
