...
from achi.full_node.mempool import Mempool
from achi.full_node.mempool_check_conditions import mempool_check_conditions_dict, get_name_puzzle_conditions
...
from achi.util.alvm import int_from_bytes
from achi.util.condition_tools import (
    pkm_pairs_for_conditions_dict,
    coin_announcements_names_for_npc,
    puzzle_announcements_names_for_npc,
)
from achi.util.errors import Err
from achi.util.generator_tools import additions_for_npc
...
class MempoolManager:
    def __init__(self, coin_store: CoinStore, consensus_constants: ConsensusConstants):
        ...
        # transactions. This prevents spam. This is equivalent to 0.055 XACH per block, or about 0.00005 XACH for two
        # spends.
        self.nonzero_fee_minimum_fpc = 5

        self.limit_factor = 0.5
        self.mempool_max_total_cost = int(self.constants.MAX_BLOCK_COST_ALVM * self.constants.MEMPOOL_BLOCK_BUFFER)
        self.potential_cache_max_total_cost = int(self.constants.MAX_BLOCK_COST_ALVM * 5)
    ...
    async def create_bundle_from_mempool(
        ...
        log.info(f"Starting to make block, max cost: {self.constants.MAX_BLOCK_COST_ALVM}")
        for dic in reversed(self.mempool.sorted_spends.values()):
            ...
            for item in dic.values():
                ...
                if (
                    item.cost + cost_sum <= self.limit_factor * self.constants.MAX_BLOCK_COST_ALVM
        ...
        if len(spend_bundles) > 0:
            log.info(
                ...
                f"full: {cost_sum / self.constants.MAX_BLOCK_COST_ALVM}"
            )
    ...
    async def pre_validate_spendbundle(self, new_spend: SpendBundle) -> NPCResult:
        ...
        cached_result_bytes = await asyncio.get_running_loop().run_in_executor(
            self.pool, get_npc_multiprocess, bytes(new_spend), self.constants.MAX_BLOCK_COST_ALVM
        )
        end_time = time.time()
    ...
    async def add_spendbundle(
        ...
        if cost > int(self.limit_factor * self.constants.MAX_BLOCK_COST_ALVM):
        ...
        log.info(
            ...
            f"({round(100.0 * cost/self.constants.MAX_BLOCK_COST_ALVM, 3)}%)"
        )
        return uint64(cost), MempoolInclusionStatus.SUCCESS, None
