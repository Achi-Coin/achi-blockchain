...
ROOT = pathlib.Path(importlib.import_module("achi").__file__).absolute().parent.parent


def solve_name_collision_problem(analysis):
    """
    There is a collision between the `achi` file name (which is the executable)
    and the `achi` directory, which contains non-code resources like `english.txt`.
    We move all the resources in the zipped area so there is no
    need to create the `achi` directory, since the names collide.
...
version_data = copy_metadata(get_distribution("achi-blockchain"))[0]
...
# TODO: collapse all these entry points into one `achi_exec` entrypoint that accepts the server as a parameter

entry_points = ["achi.cmds.achi"] + [f"achi.server.start_{s}" for s in SERVERS]
...
if THIS_IS_WINDOWS:
    entry_points.extend(["aiohttp", "achi.util.bip39"])

if THIS_IS_WINDOWS:
    achi_mod = importlib.import_module("achi")
    dll_paths = ROOT / "*.dll"
...
datas.append((f"{ROOT}/achi/util/english.txt", "achi/util"))
datas.append((f"{ROOT}/achi/util/initial-config.yaml", "achi/util"))
datas.append((f"{ROOT}/achi/wallet/puzzles/*.hex", "achi/wallet/puzzles"))
datas.append((f"{ROOT}/achi/ssl/*", "achi/ssl"))
...
add_binary("achi", f"{ROOT}/achi/cmds/achi.py", COLLECT_ARGS)
add_binary("daemon", f"{ROOT}/achi/daemon/server.py", COLLECT_ARGS)

for server in SERVERS:
    add_binary(f"start_{server}", f"{ROOT}/achi/server/start_{server}.py", COLLECT_ARGS)
