...
from achi.plotting.plot_tools import add_plot_directory, stream_plot_info_ph, stream_plot_info_pk
...
from achi.util.config import config_path_for_filename, load_config
from achi.util.keychain import Keychain
from achi.util.path import mkdir
from achi.wallet.derive_keys import master_sk_to_farmer_sk, master_sk_to_local_sk, master_sk_to_pool_sk
...
def get_farmer_public_key(alt_fingerprint: Optional[int] = None) -> G1Element:
    ...
    if sk_ent is None:
        raise RuntimeError("No keys, please run 'achi keys add', 'achi keys generate' or provide a public key with -f")
...
def get_pool_public_key(alt_fingerprint: Optional[int] = None) -> G1Element:
    ...
    if alt_fingerprint is not None:
        ...
        sk_ent = keychain.get_first_private_key()
    if sk_ent is None:
        raise RuntimeError("No keys, please run 'achi keys add', 'achi keys generate' or provide a public key with -p")
