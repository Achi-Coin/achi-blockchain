...
from achi.daemon.server import WebSocketServer, create_server_for_daemon, daemon_launch_lock_path, singleton
from achi.full_node.full_node_api import FullNodeAPI
from achi.server.start_farmer import service_kwargs_for_farmer
from achi.server.start_full_node import service_kwargs_for_full_node
from achi.server.start_harvester import service_kwargs_for_harvester
from achi.server.start_introducer import service_kwargs_for_introducer
from achi.server.start_service import Service
from achi.server.start_timelord import service_kwargs_for_timelord
from achi.server.start_wallet import service_kwargs_for_wallet
from achi.simulator.start_simulator import service_kwargs_for_full_node_simulator
from achi.timelord.timelord_launcher import kill_processes, spawn_process
from achi.types.peer_info import PeerInfo
from achi.util.bech32m import encode_puzzle_hash
from achi.util.block_tools import BlockTools, test_constants
from achi.util.hash import std_hash
from achi.util.ints import uint16, uint32
...
async def setup_farmer(
    ...
    config["xach_target_address"] = encode_puzzle_hash(b_tools.farmer_ph, "xach")
    config["pool_public_keys"] = [bytes(pk).hex() for pk in b_tools.pool_pubkeys]
    config["port"] = port
    config_pool["xach_target_address"] = encode_puzzle_hash(b_tools.pool_ph, "xach")
