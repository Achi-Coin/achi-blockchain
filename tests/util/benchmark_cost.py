...
from achi.types.blockchain_format.program import Program, INFINITE_COST
...
from achi.wallet.puzzles.p2_delegated_puzzle import puzzle_for_pk
...
def run_and_return_cost_time(achilang):

    start = time.time()
    alvm_loop = "((c (q ((c (f (a)) (c (f (a)) (c (f (r (a))) (c (f (r (r (a))))"
    " (q ()))))))) (c (q ((c (i (f (r (a))) (q (i (q 1) ((c (f (a)) (c (f (a))"
    " (c (- (f (r (a))) (q 1)) (c (f (r (r (a)))) (q ()))))))"
    " ((c (f (r (r (a)))) (q ()))))) (q (q ()))) (a)))) (a))))"
    loop_program = Program.to(binutils.assemble(alvm_loop))
    alvm_loop_solution = f"(1000 {achilang})"
    solution_program = Program.to(binutils.assemble(alvm_loop_solution))
...
def benchmark_all_operators():
    ...
    if_alvm = "(i (= (q 1000000000) (q 1000000000)) (q 1000000000) (q 1000000000))"
    ...
    if_cost, if_time = run_and_return_cost_time(if_alvm)
...
if __name__ == "__main__":
    """
    Naive way to calculate cost ratio between vByte and ALVM cost unit.
    AggSig has assigned cost of 20vBytes, simple ALVM program is benchmarked against it.
    """
    ...
    print(f"Puzzle cost sum is: {alvm_cost}")
    ...
    # alvm_should_cost = agg_sig_cost * puzzle_time / agg_sig_time
    alvm_should_cost = (agg_sig_cost * puzzle_time) / agg_sig_time
    print(f"Puzzle should cost: {alvm_should_cost}")
    constant = alvm_should_cost / alvm_cost
    format = float_to_str(constant)
    print(f"Constant factor: {format}")
    print(f"ALVM RATIO MULTIPLIER: {1/constant}")
