...
from achi.util.generator_tools import tx_removals_and_additions
from achi.util.ints import uint64, uint32
...
def get_future_reward_coins(block: FullBlock) -> Tuple[Coin, Coin, Coin]:
    timelord_amount = calculate_timelord_reward(block.height)
    ...
    return timelord_coin, pool_coin, farmer_coin
...
class TestCoinStore:
    ...
    async def test_basic_coin_store(self):
        ...
        for cache_size in [0]:
            ...
            for block in blocks:
                timelord_coin, farmer_coin, pool_coin = get_future_reward_coins(block)
                ...
                if block.is_transaction_block():
                    if block.transactions_generator is not None:
                        ...
                        npc_result = get_name_puzzle_conditions(block_gen, bt.constants.MAX_BLOCK_COST_ALVM, False)
