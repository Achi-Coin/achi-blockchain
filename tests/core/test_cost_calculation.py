...
from achi.consensus.condition_costs import ConditionCost
...
from achi.full_node.mempool_check_conditions import get_name_puzzle_conditions, get_puzzle_and_solution_for_coin
...
from achi.wallet.puzzles import p2_delegated_puzzle_or_hidden_puzzle
...
def large_block_generator(size):
    ...
    try:
        ...
        #  TODO: Re-enable large-block*.hex but cache in ~/.achi/subdir
...
class TestCostCalculation:
    ...
    async def test_basics(self):
        ...
        npc_result: NPCResult = get_name_puzzle_conditions(program, test_constants.MAX_BLOCK_COST_ALVM, False)
        ...
        error, puzzle, solution = get_puzzle_and_solution_for_coin(
            program, coin_name, test_constants.MAX_BLOCK_COST_ALVM
        )
    ...
    async def test_strict_mode(self):
        ...
        npc_result: NPCResult = get_name_puzzle_conditions(generator, test_constants.MAX_BLOCK_COST_ALVM, True)
        assert npc_result.error is not None
        npc_result: NPCResult = get_name_puzzle_conditions(generator, test_constants.MAX_BLOCK_COST_ALVM, False)
        assert npc_result.error is None

        coin_name = npc_result.npc_list[0].coin_name
        error, puzzle, solution = get_puzzle_and_solution_for_coin(
            generator, coin_name, test_constants.MAX_BLOCK_COST_ALVM
        )
        assert error is None

    @pytest.mark.asyncio
    async def test_alvm_strict_mode(self):
        block = Program.from_bytes(bytes(SMALL_BLOCK_GENERATOR.program))
        disassembly = binutils.disassemble(block)
        # this is a valid generator program except the first alvm
    ...
    async def test_tx_generator_speed(self):
        ...
        generator = BlockGenerator(program, [])
        npc_result = get_name_puzzle_conditions(generator, test_constants.MAX_BLOCK_COST_ALVM, False)
        end_time = time.time()
    ...
    @pytest.mark.asyncio
    async def test_alvm_max_cost(self):
        ...
        # ("0xfe"). In strict mode, this should fail, but in non-strict
        # mode, the unknown operator should be treated as if it returns ().
        # the ALVM program has a cost of 391969
        program = SerializedProgram.from_bytes(
        ...
        assert npc_result.error is None
        assert npc_result.alvm_cost > 10000000
    ...
    async def test_standard_tx(self):
        ...
        for i in range(0, 1000):
            cost, result = puzzle_program.run_with_cost(test_constants.MAX_BLOCK_COST_ALVM, solution_program)
            total_cost += cost
