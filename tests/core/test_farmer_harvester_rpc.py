...
from achipos import DiskPlotter

from achi.consensus.coinbase import create_puzzlehash_for_pk
from achi.plotting.plot_tools import stream_plot_info_ph, stream_plot_info_pk
from achi.protocols import farmer_protocol
from achi.rpc.farmer_rpc_api import FarmerRpcApi
from achi.rpc.farmer_rpc_client import FarmerRpcClient
from achi.rpc.harvester_rpc_api import HarvesterRpcApi
from achi.rpc.harvester_rpc_client import HarvesterRpcClient
from achi.rpc.rpc_server import start_rpc_server
...
from achi.util.block_tools import get_plot_dir
...
from achi.wallet.derive_keys import master_sk_to_wallet_sk
...
class TestRpc:
    ...
    async def test1(self, simulation):
        ...
        try:
            ...
            await client.set_reward_targets(encode_puzzle_hash(new_ph, "xach"), encode_puzzle_hash(new_ph_2, "xach"))
            ...
            await client.set_reward_targets(None, encode_puzzle_hash(new_ph_3, "xach"))
            ...
            assert config["farmer"]["xach_target_address"] == encode_puzzle_hash(new_ph, "xach")
            assert config["pool"]["xach_target_address"] == encode_puzzle_hash(new_ph_3, "xach")

            new_ph_3_encoded = encode_puzzle_hash(new_ph_3, "xach")
