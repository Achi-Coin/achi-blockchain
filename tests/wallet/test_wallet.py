...
from achi.protocols.full_node_protocol import RespondBlock
from achi.server.server import AchiServer
from achi.simulator.simulator_protocol import FarmNewBlockProtocol, ReorgProtocol
from achi.types.peer_info import PeerInfo
from achi.util.ints import uint16, uint32, uint64
...
class TestWalletSimulator:
    ...
    async def test_wallet_coinbase(self, wallet_node):
        ...
        funds = sum(
            [
                calculate_timelord_reward(uint32(i)) + calculate_pool_reward(uint32(i)) +
    ...
    async def test_wallet_make_transaction(self, two_wallet_nodes):
        ...
        funds = sum(
            [calculate_timelord_reward(uint32(i)) + calculate_pool_reward(uint32(i)) +
             calculate_base_farmer_reward(uint32(i)) for i in range(1, num_blocks)]
        )
    ...
    async def test_wallet_coinbase_reorg(self, wallet_node):
        ...
        funds = sum(
            [calculate_timelord_reward(uint32(i)) + calculate_pool_reward(uint32(i)) + calculate_base_farmer_reward(uint32(i)) for i in range(1, num_blocks)]
        )
    ...
    #         outbound: List[WSAchiConnection] = wallet.server.get_outgoing_connections()
